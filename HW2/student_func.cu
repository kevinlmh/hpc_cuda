...
#include "utils.h"
// include cmath library since im using max and min function
#include <cmath>
                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  // the 2D position of this thread, aka absolute image position
  ...
  for (int filter_y = -filterWidth/2; filter_y <= filterWidth/2; ++filter_y) {
	for (int filter_x = -filterWidth/2; filter_x <= filterWidth/2; ++filter_x) {
	  ...
	  int image_y = min(max(thread_2D_pos.y + filter_y, 0), static_cast<int>(numRows - 1));
	  int image_x = min(max(thread_2D_pos.x + filter_x, 0), static_cast<int>(numCols - 1));
                      ...
                      unsigned char* const blueChannel)
{
  ...
  // the 1D position of this thread
  ...
  // split the uchar4 RGBA image into 3 color channels
                        ...
                        const int filterWidth)
{
  ...
  blockSize.x = (numCols%16 == 0) ? numCols/16 : numCols/16+1;  
  blockSize.y = (numRows%16 == 0) ? numRows/16 : numRows/16+1;
